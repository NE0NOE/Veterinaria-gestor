...
const NavItem: React.FC<NavItemProps> = ({ to, icon: Icon, label, requiredPermission, currentPath, userRole }) => {
  ...
  return (
    <li>
      <Link
        to={to}
        className={`flex items-center p-3 rounded-lg text-gray-700 hover:bg-indigo-100 hover:text-indigo-700 transition-colors duration-150 ${
  ...
  return (
    <aside className="w-64 bg-white h-screen fixed top-0 left-0 shadow-lg flex flex-col">
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500 mt-1">Rol: {user.role}</p> {/* Kept Rol in Spanish */}
      </div>
      ...
      <div className="p-4 border-t border-gray-200">
        ...
        <button
          ...
          className="w-full flex items-center p-3 rounded-lg text-red-600 hover:bg-red-100 transition-colors duration-150 mt-2"
        >
